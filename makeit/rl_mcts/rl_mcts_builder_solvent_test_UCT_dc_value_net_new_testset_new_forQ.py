...
#RL_PROB_MODEL_PATH = 'makeit/models/rl_prob_solvent_56_csolv_2_1/'
#RL_VAL_MODEL_PATH = 'makeit/models/rl_val_with_pure_normalized_csolvscore_2/'

RL_VAL_MODEL_PATH = 'makeit/models/rl_val_retrain_5_2/'
#RL_VAL_MODEL_PATH = 'makeit/models/rl_val_csolvscore_retrain_round_2_5/'
...
############important method coordinate, basically executed the majority of tree search work #########
					...
					for smi, indices, prob, value in reactants:
						...
						if smi not in self.Chemicals:
							...
							if ppg is not None and ppg > 0:
								self.Chemicals[smi].set_price(ppg) # z set 1, price 1 s_Q 0
	...
	def UCB(self, chem_smi, c_exploration=0.2, c_solv=0.01, path=[]): #updated by xiaoxue at 12072018
		...
		if chem_smi == self.smiles and len(C.reactions)==0: #the intial case
			template_id = C.indices[0]
			Q_sa = 0###changed from - to + used to be 0.1
			U_sa = 0###no prob, pure UCT
			score = Q_sa + U_sa
			#print(Q_sa, U_sa)
			...
			for template_id in C.reactions:
				...
				try:
					...
					if Q_sa/c_exploration > 2:
						c_exploration = Q_sa/2
					##################################################################
					U_sa = c_exploration * np.sqrt(2* np.log(product_visits+0.01) / (1+R.visit_count))#####
				except:
		...
		# unexpanded template
			if (len(C.reactions) < self.max_branching and len(C.reactions) < len(C.indices)):# or chem_smi == self.smiles:
				...
				#################################################################
				U_sa = c_exploration * np.sqrt(2*np.log(product_visits+0.01))#######
				score = Q_sa + U_sa
				# print('Qsa and Usa are:', Q_sa, U_sa)
				rxn_scores.append((score, template_id))
		...
		if len(rxn_scores) > 0:
			...
			# print(chem_smi)
			# for i,(score,tid) in enumerate(sorted_rxn_scores):
			#   if i > 3:
			#       break
			#   s = '\t%d score:%.3f '%(tid, score)
			#   if tid not in C.reactions:
			#       s += 'new_template'
			#   else:
			#       R = C.reactions[tid]
			#       s += 'es_price:%.2f #:%d '%(R.estimate_price, R.visit_count)
			#       s += 'done:%s ' % R.done
			#       if i == 0:
			#           s += str(R.reactant_smiles)
			#   print(s)

			# print('\t... tried %d reactions, %d successful' % (len(C.reactions), len(sorted_rxn_scores) - 1))
...
# 			C.z = gamma * max([C.reactions[tid].c_z for tid in C.reactions])######flipped C.z and R.c_z compared to brand_new_3.py
 ################################################################################################33           
		...
		# ############################solvent scores adding up in the Bellman equation style######################################
		# 	if R.price != -1:
		# 		s_Q = 0.9*sum([self.Chemicals[smi].s_Q for smi in R.reactant_smiles])
		# 		R.update_for_s_Q(s_Q)######
		# 		if C.s_Q == -1. or C.s_Q < R.for_s_Q:
		# 			C.s_Q = R.for_s_Q
# ############################def 2: solvent scores average up in the Bellman equation style######################################
# 			if R.price != -1:
# 				if all([self.Chemicals[smi].steps >= 0 for smi in R.reactant_smiles]):
# 					s_Q = 0.9*sum([self.Chemicals[smi].s_Q * self.Chemicals[smi].steps for smi in R.reactant_smiles])
# 					sum_c_steps = sum([self.Chemicals[smi].steps for smi in R.reactant_smiles])
# 					R.for_s_Q = (R.s_Q + s_Q)/(1+sum_c_steps)
# 					if C.s_Q == -1. or C.s_Q < R.for_s_Q:
# 						C.s_Q = R.for_s_Q
# 						C.steps = 1 + sum([self.Chemicals[smi].steps for smi in R.reactant_smiles])
# 						C.best_solvent_template = template_id 
############################def 3 penalty######################################
			if R.price != -1:
				if all([self.Chemicals[smi].s_Q <= 0 for smi in R.reactant_smiles]):
					s_Q = sum([self.Chemicals[smi].s_Q for smi in R.reactant_smiles])
					R.for_s_Q = (0.45*R.s_Q - 0.55) + s_Q######
					if C.s_Q == 1. or C.s_Q < R.for_s_Q:
						C.s_Q = R.for_s_Q
						C.best_solvent_template = template_id
		#################################################################################
	...
	def build_tree(self):
		...
		print("=== max tree_solvent_score: %.4f" %C.s_Q)
...
# 		return trees#(tree_status, trees)
...
if __name__ == '__main__':
	...
	Tree = MCTS(nproc=NCPUS,max_branching=max_branching, mincount=gc.RETRO_TRANSFORMS_CHIRAL['mincount'], 
		mincount_chiral=gc.RETRO_TRANSFORMS_CHIRAL['mincount_chiral'], solvent_evaluation = True)
	...
	# print(type(paths))
	# print(len(paths))
	#print(paths)#paths is a list, which elements are netted dictionaries
	# num_trees=0
...
# # #     ####################################################################################
	...
	# N = 500
	# smiles_list = [line.strip().split('.')[0] for line in
	...
	success = 0
	total = 0
	list_smi_tree_solv_score = []
	#all_paths = []


	# ########### STAGE 1 - PROCESS ALL CHEMICALS
	with open('makeit/data/Trees_UCT_dc_value_net_rl_val_5_2_30s_def_3_testset_later_2000.pkl', 'wb') as fid:
		for _id, smiles in enumerate(smiles_list[2001:4001]): 
			...
			if Chemicals[smiles].price != -1:
				success += 1
				list_smi_tree_solv_score.append((smiles, Chemicals[smiles].s_Q))
				print('saving: {}, max tree_solvent_score: {}'.format(smiles, Chemicals[smiles].s_Q))
				#all_paths.append(paths)
				#print('paths saved')
			print('After looking at chemical index {}'.format(_id))
			print('Success ratio: %f (%d/%d)' % (float(success)/total, success, total)) 

		pickle.dump((list_smi_tree_solv_score), fid)
