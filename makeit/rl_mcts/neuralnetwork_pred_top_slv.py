...
class NeuralNetContextRecommender(ContextRecommender):
    ...
    def load_nn_model(self, model_path="", info_path="", weights_path=""):
        ...
        slv_sc_dict_file = info_path + "slv_sc_dict.pickle"
        ...
        with open(slv_sc_dict_file,"rb") as SLV_F:
            self.slv_sc_dict = pickle.load(SLV_F)    
    ...
    def predict_top_solvents_from_rxn_smiles(self, rxn):
 
    #     rsmi_new = '.'.join(rsmi)
    #     psmi_new = '.'.join(psmi)
    #     rp_list = [rsmi_new, psmi_new]
        # rxn='>>'.join(rp_list)
        rsmi = rxn.split('>>')[0]
        psmi = rxn.split('>>')[1]
        rsmi_list=rsmi.split('.')
        psmi_list=psmi.split('.')
        return self.predict_top_solvents_from_split_smiles(rsmi_list, psmi_list)


    def predict_top_solvents_from_split_smiles(self, rsmi_list=[], psmi_list=[]):
        # try:
        # rsmi = rxn.split('>>')[0]#str
        # psmi = rxn.split('>>')[1]#str
        rsmi = '.'.join(rsmi_list)
        psmi = '.'.join(psmi_list)
        rct_mol = Chem.MolFromSmiles(rsmi)
        prd_mol = Chem.MolFromSmiles(psmi)
        ...
        (top_combos,top_combo_scores)=self.predict_top_combos(inputs=inputs,return_categories_only = True,c1_rank_thres=1, s1_rank_thres=3, s2_rank_thres=1, r1_rank_thres=1, r2_rank_thres=1)
        tot_score = 0
        tot_prob = 0
        #sep_score = collections.defaultdict(int)#
        for i in range(len(top_combos)):
            #sep_score[top_combos[i][1]]+=top_combo_scores[i]#
            tot_score += self.slv_sc_dict[top_combos[i][1]]*top_combo_scores[i]
            tot_prob += top_combo_scores[i]
        avg_slv_sc = float(tot_score)/tot_prob
        #sep_score = {key:value/tot_prob for key,value in sep_score.items()}#
        # c = list(zip(top_combos, top_combo_scores))#
        # c.sort(key=lambda x : x[1])#
        # top_combos, top_combo_scores = zip(*c)######top_combos#

        names = [self.s1_dict[combo[1]] for combo in top_combos]
        return(avg_slv_sc,names)#,[x[1] for x in top_combos], sep_score)#when used in tree expander, delete top_combos and sep_score

        # except:
        #     print("cannot resolve reaction {}".format(rxn))
...
if __name__ == '__main__':
    ...
    cont.load_nn_model(model_path=gc.NEURALNET_CONTEXT_REC['model_path'], info_path=gc.NEURALNET_CONTEXT_REC[
                       'info_path'], weights_path=gc.NEURALNET_CONTEXT_REC['weights_path'])
    #print(cont.predict_top_solvents_from_rxn_smiles('Cn1c(=O)ccc2cccc(OCc3ccccc3)c21>>Cn1c(=O)ccc2cccc(O)c21'))
    print(cont.predict_top_solvents_from_rxn_smiles('Nc1ccccc1N.O=c1oc2ccccc2c(O)c1[N+](=O)[O-]>>Nc1ccccc1Nc1c([N+](=O)[O-])c(=O)oc2ccccc12'))
    #print(cont.predict_top_solvents_from_split_smiles(rsmi_list=['CC1(C)OBOC1(C)C','Cc1ccc(Br)cc1'],psmi_list=['Cc1cccc(B2OC(C)(C)C(C)(C)O2)c1']))
    # # print(cont.name_to_category('c1','Reaxys Name palladium on activated charcoal'))
    # print(cont.predict_top_solvents_from_rxn_smiles('Cc1ccnc(NC2CC2)c1NC(=O)c1cccnc1Cl>>Cc1ccnc2c1NC(=O)c1cccnc1N2C1CC1'))
    # #(-0.8501232475984415, [u'C1COCCO1', u'Cc1ccccc1', u'C1CCOC1'])
    # #(-0.19491153987348167, ['', u'CN(C)C=O', u'C1CCOC1'])
    # print(cont.predict_top_solvents_from_split_smiles(rsmi_list=['Cc1ccnc(NC2CC2)c1NC(=O)c1cccnc1Cl'],psmi_list=['Cc1ccnc2c1NC(=O)c1cccnc1N2C1CC1']))