...
retro_transformer_loc = 'rl_retro_transformer'


class RLRetroTransformer(TemplateTransformer):
    """
    ...
    def __init__(self, celery=False, mincount=gc.RETRO_TRANSFORMS_CHIRAL['mincount'], 
        ...
        super(RLRetroTransformer, self).__init__()
    ...
    def get_outcomes(self, smiles, mincount, template_id, **kwargs):
        ...
        for smiles_list in self.apply_one_template(mol, smiles, template):
    ...
    def apply_one_template(self, react_mol, smiles, template, **kwargs):
        ...
        for j, outcome in enumerate(outcomes):
            ...
            results.append(sorted(smiles_list))
...
if __name__ == '__main__':
    ...
    t = RLRetroTransformer()
    t.load(chiral=True, refs=False, rxns=True)

    outcomes = t.get_outcomes('CCOC(=O)[C@H]1C[C@@H](C(=O)N2[C@@H](c3ccccc3)CC[C@@H]2c2ccccc2)[C@@H](c2ccccc2)N1', 100,109659,apply_fast_filter=True, filter_threshold=0.75)
