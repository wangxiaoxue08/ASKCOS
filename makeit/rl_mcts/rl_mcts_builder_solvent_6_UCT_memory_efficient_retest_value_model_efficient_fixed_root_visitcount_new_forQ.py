...
############important method coordinate, basically executed the majority of tree search work #########
	...
	def coordinate(self):
		...
		while self.Chemicals[self.smiles].visit_count <= self.max_root_visit_count:
			visit_count_this_round = self.Chemicals[self.smiles].visit_count##############
			STUCK = False##############
			#if (int(elapsed_time)/10 == next):
			if (int(self.Chemicals[self.smiles].visit_count)/500 == next):
				next += 1
				#print "Worked for {}/{} s".format(int(elapsed_time*10)/10.0, self.expansion_time)
				print "Worked for {}/{} visit_count".format(int(self.Chemicals[self.smiles].visit_count*10)/10.0, self.max_root_visit_count)
			...
			for _id in range(self.nproc):
				if len(self.pathways[_id]) == 0:
					...
					if self.Chemicals[self.smiles].visit_count <= visit_count_this_round:
						print('process stuck')
						STUCK = True
						break
...
# 			C.z = gamma * max([C.reactions[tid].c_z for tid in C.reactions])######flipped C.z and R.c_z compared to brand_new_3.py
 ################################################################################################33           
	...
	def build_tree(self):
		...
		print("=== time for first pathway: %.2fs" % self.time_for_first_path)
	...
	def get_buyable_paths(self, 
							...
							max_depth=10, 
							expansion_time = 120,
							nproc=8, ngpus=1, mincount=25, chiral=True, max_root_visit_count = 5000):
		...
		self.max_root_visit_count = max_root_visit_count
		self.nproc = nproc
		self.ngpus = ngpus
...
if __name__ == '__main__':
	...
	max_root_visit_count = 5000
	
	smiles_id = 0
	smiles = "C1=CC(=C(C=C1F)F)C(CN2C=NC=N2)(CN3C=NC=N3)O"
	smiles = 'CCCC1C[C@@H](C(=O)O)N(C)C1'
	#Load tree builder 
	NCPUS = 1
	NGPUS = 1 
	...
	Tree.get_buyable_paths(smiles, smiles_id, 
										...
										expansion_time=simulation_time, max_root_visit_count = 5000)
...
# # #     ####################################################################################
	...
	for _id, smiles in enumerate(smiles_list[0:393]+smiles_list[394:498]+smiles_list[499:533]+smiles_list[534:N]): 
		smiles = Chem.MolToSmiles(Chem.MolFromSmiles(smiles), True)
		smiles_id=_id
		Tree.get_buyable_paths(smiles,smiles_id,
												nproc=NCPUS,
												expansion_time=simulation_time, max_root_visit_count = max_root_visit_count)
